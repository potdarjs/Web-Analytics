...
# This section lists some examples of public HTTP APIs that publish data in JSON format. 
# These are great to get a sense of the complex structures that are encountered in real 
# world JSON data. All services are free, but some require registration/authentication. 
# Each example returns lots of data, therefore not all output is printed in this document.



# Github is an online code repository and has APIs to get live data on almost all activity.
# Below some examples from a well known R package and author:
...
# CitiBike NYC
# A single public API that shows location, status and current availability for 
# all stations in the New York City bike sharing imitative.
...
# Ergast
# The Ergast Developer API is an experimental web service which provides a historical 
...
# ProPublica
# Below an example from the ProPublica Nonprofit Explorer API where we 
# retrieve the first 10 pages of tax-exempt organizations in the USA, ordered by revenue. 
# The rbind_pages function is used to combine the pages into a single data frame.
...
# New York Times
# The New York Times has several APIs as part of the NYT developer network. 
# These interface to data from various departments, such as news articles, 
# book reviews, real estate, etc. 
# Registration is required (but free) and a key can be obtained at 
# http://developer.nytimes.com/signup. 
# The code below includes some example keys for illustration purposes.
...
#Sunlight Foundation
# The Sunlight Foundation is a non-profit that helps to make government 
# transparent and accountable through data, tools, policy and journalism. Register a free key at here. An example key is provided.
